...
//             Review, approve, or delete customer feedback submitted through your
//             website.
//           </p>
//         </div>
...
// Assuming these are the project's custom UI components (Shadcn/Tailwind based)
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import {
  AlertDialog,
...
export default function AdminReviewsPage() {
  ...
  const fetchReviews = useCallback(async () => {
    setIsLoading(true);
    try {
      // Mock Data Structure for IReview for environment purposes if needed, but keeping actual import
      // interface IReview { _id: string; name: string; email: string; rating: number; review: string; isApproved: boolean; createdAt: string; }
      
      // In a real project, this API call would return actual data:
      const response = await axios.get("/api/reviews");
      setReviews(response.data.reviews || []);
    } catch (error) {
      console.error("Failed to fetch reviews:", error);
  ...
  const RatingBar = ({
    ...
    <div className="flex items-center space-x-4 text-sm">
      ...
      <div className="w-12 text-right flex-shrink-0 font-medium text-white">
        {percentage}%
      </div>
      <div className="w-10 text-right flex-shrink-0 text-gray-400 text-xs">
        ({count})
      </div>
    </div>
  );
