...
import { Toaster } from "@/components/ui/sonner";
import { toast } from "sonner";
import { v4 as uuidv4 } from 'uuid'; // Although not directly used in this page now, it might be used globally or in other components.
import { useRouter } from 'next/navigation';
import { useSession } from 'next-auth/react';
...
const ProductsPage = ({ groupedProducts, handleAddToCart, filterName, setFilterName, sortOrder, setSortOrder, setCurrentPage, getCartItemCount, loading, error }: any) => (
  <div className="w-full">
    ...
    >
      <div className="absolute inset-0 bg-black/60 rounded-b-[40px]"></div>
      <div className="relative z-10 text-center">
        <h2 className="text-5xl tracking-tight md:text-6xl drop-shadow-2xl font-[Yeseve One] text-white">Our Products</h2>
        <p className="mt-4 text-lg md:text-xl font-medium drop-shadow-lg font-[Varela Round] text-white">Delicious food, made with love.</p>
      </div>
    </div>
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const response = await axios.get('/api/product'); // Ensure this path is correct for your product API
        const data = response.data;
    ...
    <>
      ...
      <div className="min-h-screen bg-[#141F2D] font-[Varela Round] text-[#EFA765] flex flex-col">
        ...
        <main className="flex-grow">
          ...
          {currentPage === 'cart' && ( // 'cart' page now represents the full checkout flow
            <CheckoutPage
              cart={cart}
              handleUpdateQuantity={handleUpdateQuantity}
              handleRemoveItem={handleRemoveItem}
              getOriginalCartTotal={getOriginalCartTotal}
              dispatch={dispatch} // Passed to allow clearing cart after successful checkout
              setCurrentPage={setCurrentPage}
            />
          )}
        </main>
