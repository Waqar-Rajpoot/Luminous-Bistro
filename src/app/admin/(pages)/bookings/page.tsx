  ...
  switch (status) {
    ...
    case "canceled":
      return {
        text: "Canceled",
        className:
          "bg-red-700/20 text-red-300 border-red-700/40 hover:bg-red-700/40",
      };
    default:
      return {
        text: "Unknown",
        className:
          "bg-gray-700/20 text-gray-300 border-gray-700/40 hover:bg-gray-700/40",
      };
  }
};
...
export default function AdminBookingsPage() {
  ...
  const [filterStatus, setFilterStatus] = useState<
    "all" | "confirmed" | "pending" | "canceled"
  >("all");
  // ---------------------------------------
  ...
  const confirmedBookingsCount = bookings.filter(
    (b) => b.isConfirmed === "confirmed"
  ).length;
  const pendingBookingsCount = bookings.filter(
    (b) => b.isConfirmed === "pending"
  ).length;
  const canceledBookingsCount = bookings.filter(
    (b) => b.isConfirmed === "canceled"
  ).length;
  const percentageConfirmed =
    totalBookings > 0
      ? ((confirmedBookingsCount / totalBookings) * 100).toFixed(0)
      : 0;
  // -----------------------------
  ...
  const handleChangeStatus = async (
    bookingId: string,
    currentStatus: IBooking["isConfirmed"]
  ) => {
    ...
    try {
       await axios.patch(`/api/bookingTable/${bookingId}`, {
      ...
      setBookings((prevBookings) =>
        prevBookings.map((booking) =>
          booking._id === bookingId
            ? ({ ...booking, isConfirmed: newStatus } as IBooking)
            : booking
        )
      );
      ...
      const axiosError = error as AxiosError<ErrorResponse>;
      toast.error(
        axiosError.response?.data.message || "Failed to update booking status."
      );
    } finally {
      ...
      <div className="flex-1 flex flex-col p-4 md:p-6 lg:p-8 min-h-screen">
        <div className="mb-8">
          ...
          <p className="text-gray-400">
            A comprehensive management dashboard for all restaurant table
            bookings.
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 p-4 rounded-lg bg-[#141f2d] border border-[#efa765]/30">
          ...
          <div className="flex space-x-2 flex-wrap justify-end">
            ...
            >
              <CheckCircle className="h-4 w-4 mr-1 inline-block" /> Confirmed (
            ...
            >
              <Clock className="h-4 w-4 mr-1 inline-block" /> Pending (
            ...
            >
              <XCircle className="h-4 w-4 mr-1 inline-block" /> Canceled (
            ...
            <Table>
              <TableHeader className="sticky top-0 bg-[#1c2a3b] text-[#efa765] shadow-md z-10">
                <TableRow className="border-gray-700">
                  ...
                  <TableHead className="text-center text-[#efa765] font-bold">
                    Guests
                  </TableHead>
                ...
                {isLoading ? (
                  ...
                  filteredBookings.map((booking) => {
                    ...
                    return (
                      <TableRow
                        key={booking._id}
                        className="border-gray-700 hover:bg-[#1c2a3b] transition-colors duration-150 cursor-pointer"
                        onClick={() => handleBookingClick(booking)}
                      >
                        <TableCell className="font-medium text-white truncate max-w-[150px]">
                          {booking.name}
                        </TableCell>
                        ...
                        <TableCell className="text-center text-gray-400">
                          <div className="flex justify-center items-center font-semibold">
                            {booking.guests}{" "}
                            <Users className="h-4 w-4 ml-1 text-[#efa765]" />
                          </div>
                        </TableCell>
                              ...
                              <AlertDialogContent className="bg-gray-900 text-white border border-red-700">
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription className="text-gray-300">
                                    ...
                                    {booking.name}** from your database.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
        ...
        <DialogContent className="sm:max-w-[425px] bg-gray-900 text-white border border-[#efa765]">
          ...
          <div className="text-gray-400 space-y-3 pt-2">
            ...
            <div className="pt-4">
              ...
              >
                {getBookingBadge(
                  selectedBooking?.isConfirmed || "pending"
                ).text.toUpperCase()}
              </Badge>
            </div>
          </div>
          <div className="bg-gray-800 p-4 rounded-md h-36 overflow-y-auto text-gray-200 border border-gray-700 mt-4">
            <span className="font-bold text-[#efa765] block mb-1">
              Special Requests:
            </span>
            <p className="mt-1 text-sm leading-relaxed">
              {selectedBooking?.requests ||
                "No special requests were provided for this reservation."}
            </p>
          </div>
        </DialogContent>
      </Dialog>
